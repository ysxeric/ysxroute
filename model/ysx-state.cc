                              ...
                              Ipv4Address const &lastAddr)
{
	 for (TopologySet::iterator it = m_topologySet.begin ();
	    ...
	    {
	      if (it->destNode.ipv4 == destAddr && it->lastNode.ipv4 == lastAddr)
	        {
	          return &(*it);
	        }
	    }
...
TopologyTuple*
OlsrState::FindTopologyTuple (const Node_id_mob& destNode,
		                           const Node_id_mob& lastNode)
{
  for (TopologySet::iterator it = m_topologySet.begin ();
       it != m_topologySet.end (); it++)
    {
      if (it->destNode == destNode && it->lastNode == lastNode)
        {
          return &(*it);
        }
    }
...
OlsrState::FindNewerTopologyTuple (Ipv4Address const & lastAddr, uint16_t ansn)
{
  for (TopologySet::iterator it = m_topologySet.begin ();
       it != m_topologySet.end (); it++)
    {
      if (it->lastNode.ipv4 == lastAddr && it->sequenceNumber > ansn)
        {
          return &(*it);
        }
    }
