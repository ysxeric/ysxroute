...
#include "ysx-routing-protocol.h"
//#include "ysx-"
...
{
  static TypeId tid = TypeId ("ns3::ysxroute::RoutingProtocol")
    ...
    .AddTraceSource ("Rx", "Receive OLSR packet.",
                     ...
                     "ns3::ysxroute::RoutingProtocol::PacketTxRxTracedCallback")
    ...
    .AddTraceSource ("RoutingTableChanged", "The OLSR routing table has changed.",
                     ...
                     "ns3::ysxroute::RoutingProtocol::TableChangeTracedCallback")
  ;
  return tid;
}
...
RoutingProtocol::SetIpv4 (Ptr<Ipv4> ipv4)
{
  NS_ASSERT (ipv4 != 0);
  NS_ASSERT (m_ipv4 == 0);
  NS_LOG_DEBUG ("Created ysxroute::RoutingProtocol");
...
RoutingProtocol::RecvOlsr (Ptr<Socket> socket)
{
  ...
  ysxroute::PacketHeader ysxPacketHeader;
  packet->RemoveHeader (ysxPacketHeader);
  NS_ASSERT (ysxPacketHeader.GetPacketLength () >= ysxPacketHeader.GetSerializedSize ());
  uint32_t sizeLeft = ysxPacketHeader.GetPacketLength () - ysxPacketHeader.GetSerializedSize ();
  ...
  m_rxPacketTrace (ysxPacketHeader, messages);
      ...
      if (duplicated == NULL)
        {
          switch (messageHeader.GetMessageType ())
            {
            case ysxroute::MessageHeader::HELLO_MESSAGE:
            ...
            case ysxroute::MessageHeader::TC_MESSAGE:
              NS_LOG_DEBUG (Simulator::Now ().GetSeconds ()
                            << "s OLSR node " << m_mainAddress
      ...
      if (do_forwarding)
        {
          ...
          if (messageHeader.GetMessageType ()  != ysxroute::MessageHeader::HELLO_MESSAGE)
            {
  ...
  for (uint32_t h = 2;; h++)
    {
      ...
      for (TopologySet::const_iterator it = topology.begin ();
           it != topology.end (); it++)
        {
          ...
          bool have_destAddrEntry = Lookup (topology_tuple.destNode.ipv4, destAddrEntry);
          bool have_lastAddrEntry = Lookup (topology_tuple.lastNode.ipv4, lastAddrEntry);
          if (!have_destAddrEntry && have_lastAddrEntry && lastAddrEntry.distance == h)
            {
              ...
              //                                    route entry where:
              //                                       R_dest_addr == T_last_addr.
              AddEntry (topology_tuple.destNode.ipv4,
...
void
RoutingProtocol::ProcessHello (const ysxroute::MessageHeader &msg,
                               const Ipv4Address &receiverIface,
                               const Ipv4Address &senderIface)
{
  NS_LOG_FUNCTION (msg << receiverIface << senderIface);

  const ysxroute::MessageHeader::Hello &hello = msg.GetHello ();
...
void
RoutingProtocol::ProcessTc (const ysxroute::MessageHeader &msg,
                            const Ipv4Address &senderIface)
{
  const ysxroute::MessageHeader::Tc &tc = msg.GetTc ();
...
//       i != tc.neighborAddresses.end (); i++)
     for(std::vector<ysxroute::MessageHeader::NodeIP_Mob>::const_iterator i = tc.neighborIP_Mob.begin();
    		 i!=tc.neighborIP_Mob.end();i++)
    {
      const Ipv4Address &addr = i->ipv4add;
      ...
      if (topologyTuple != NULL)
      ...
      else
        {
          ...
          m_events.Track (Simulator::Schedule (DELAY (topologyTuple.expirationTime),
                                               ...
                                               this,
                                               topologyTuple.destNode.ipv4,
                                               topologyTuple.lastNode.ipv4));
        }
    }
...
void
RoutingProtocol::ProcessMid (const ysxroute::MessageHeader &msg,
                             const Ipv4Address &senderIface)
{
  const ysxroute::MessageHeader::Mid &mid = msg.GetMid ();
  Time now = Simulator::Now ();
...
void
RoutingProtocol::ProcessHna (const ysxroute::MessageHeader &msg,
                             const Ipv4Address &senderIface)
{

  const ysxroute::MessageHeader::Hna &hna = msg.GetHna ();
  Time now = Simulator::Now ();
  ...
  for (std::vector<ysxroute::MessageHeader::Hna::Association>::const_iterator it = hna.associations.begin ();
...
void
RoutingProtocol::ForwardDefault (ysxroute::MessageHeader olsrMessage,
...
void
RoutingProtocol::QueueMessage (const ysxroute::MessageHeader &message, Time delay)
{
...
RoutingProtocol::SendQueuedMessages ()
{
  ...
  for (std::vector<ysxroute::MessageHeader>::const_iterator message = m_queuedMessages.begin ();
...
RoutingProtocol::SendHello ()
{
  ...
  ysxroute::MessageHeader::Hello &hello = msg.GetHello ();

  hello.SetHTime (m_helloInterval);
  hello.willingness = m_willingness;

  //Set Position and velocity;By Yangshixin
  Ptr<MobilityModel> mo=m_ipv4->GetObject<Node>()->GetObject< MobilityModel>();
  Vector pos = mo->GetPosition ();
...
//		  <<UinttoInt(hello.vel_x)<<",vel_y:"<<UinttoInt(hello.vel_y)<<std::endl);
  ...
  for (LinkSet::const_iterator link_tuple = links.begin ();
       link_tuple != links.end (); link_tuple++)
    {
      ...
      ysxroute::MessageHeader::Hello::LinkMessage linkMessage;
...
RoutingProtocol::SendTc ()
{
  ...
  ysxroute::MessageHeader::Tc &tc = msg.GetTc ();
  tc.ansn = m_ansn;
  Ptr<MobilityModel> mob=m_ipv4->GetObject<Node>()->GetObject< MobilityModel>();
...
//      tc.neighborAddresses.push_back (mprsel_tuple->mainAddr);
	  ysxroute::MessageHeader::NodeIP_Mob temp;
...
{
  ysxroute::MessageHeader msg;
  ysxroute::MessageHeader::Mid &mid = msg.GetMid ();
...
RoutingProtocol::SendHna ()
{
  ...
  msg.SetHopCount (0);
  msg.SetMessageSequenceNumber (GetMessageSequenceNumber ());
  ysxroute::MessageHeader::Hna &hna = msg.GetHna ();

  std::vector<ysxroute::MessageHeader::Hna::Association> &associations = hna.associations;
  ...
  for (Associations::const_iterator it = localHnaAssociations.begin ();
    ...
    {
      ysxroute::MessageHeader::Hna::Association assoc = { it->networkAddr, it->netmask};
...
void
RoutingProtocol::LinkSensing (const ysxroute::MessageHeader &msg,
                              ...
                              const Ipv4Address &senderIface)
{
  ...
  for (std::vector<ysxroute::MessageHeader::Hello::LinkMessage>::const_iterator linkMessage =
...
void
RoutingProtocol::PopulateNeighborSet (const ysxroute::MessageHeader &msg,
                                      const ysxroute::MessageHeader::Hello &hello)
{
...
void
RoutingProtocol::PopulateTwoHopNeighborSet (const ysxroute::MessageHeader &msg,
                                            const ysxroute::MessageHeader::Hello &hello)
{
  Time now = Simulator::Now ();
  ...
  for (LinkSet::const_iterator link_tuple = m_state.GetLinks ().begin ();
       link_tuple != m_state.GetLinks ().end (); link_tuple++)
    {
      ...
      typedef std::vector<ysxroute::MessageHeader::Hello::LinkMessage> LinkMessageVec;
...
void
RoutingProtocol::PopulateMprSelectorSet (const ysxroute::MessageHeader &msg,
                                         const ysxroute::MessageHeader::Hello &hello)
{
  NS_LOG_FUNCTION (this);
...
///
void
ysxroute::mac_failed (Ptr<Packet> p)
{
  ...
  debug ("%f: Node %d MAC Layer detects a breakage on link to %d\n",
         now,
         ysxroute::node_id (ra_addr ()),
         ysxroute::node_id (ch->next_hop ()));
...
//         Simulator::Now (),
//         ysxroute::node_id(ra_addr()),
//         ysxroute::node_id(tuple->neighborMainAddr),
...
//         ysxroute::node_id(ra_addr()),
//         ysxroute::node_id(tuple->neighborMainAddr),
//         ysxroute::node_id(tuple->twoHopNeighborAddr));
...
//         Simulator::Now (),
//         ysxroute::node_id(ra_addr()),
//         ysxroute::node_id(tuple->main_addr()));
...
//         Simulator::Now (),
//         ysxroute::node_id(ra_addr()),
//         ysxroute::node_id(tuple->dest_addr()),
//         ysxroute::node_id(tuple->last_addr()),
//         tuple->seq());
...
//         ysxroute::node_id(ra_addr()),
//         ysxroute::node_id(tuple->main_addr()),
//         ysxroute::node_id(tuple->iface_addr()));
...
RoutingProtocol::TopologyTupleTimerExpire (Ipv4Address destAddr, Ipv4Address lastAddr)
{
  ...
  if (tuple->expirationTime < Simulator::Now ())
  ...
  else
    {
      m_events.Track (Simulator::Schedule (DELAY (tuple->expirationTime),
                                           &RoutingProtocol::TopologyTupleTimerExpire,
                                           this, tuple->destNode.ipv4, tuple->lastNode.ipv4));
    }
}
